...
### Djikstra's Algorithm
    * finds the shortest path in between two nodes in a graph
    * greedy algorithm
    * can either return a single distance between two nodes or a shortest path tree.
    * worst case performance: O(|V|^2) (no minimum priority queue), O(|E| + |V| log |V|) (minimum priority queue used)
    * algorithm:
        1. Pick an initial node. Mark it as current node.
        2. For the current node, look at all neighbor nodes and assign them a distance value (running distance plus distance from current node to unvisited node). Mark them all unvisited. Put them in the unvisited set.
        3. Mark the current node as visited and remove it from the unvisited set.
        4. If the destination node is marked as visited (when looking for a single shortest path) or if there are no nodes with a finite distance away in the unvisited set (no unvisited available connections) then stop.
        5. Pick the node with the shortest distance from the unvisited set. Mark it as current and go to step 3.

### Prim's Algorithm
    * finds minimum spanning tree for a weighted undirected graph.
        - a minimum spanning tree includes a subset of edges that contain every vertex in the tree with the minimum cost available.
    * greedy algorithm
    * algorithm:
        1. Initialize a tree with a single vertex chosen from the graph.
        2. Of the edges that connect the tree to vertices not in the tree, find the minimum weight edge and transfer it to the tree.
        3. Repeat step 2 until all vertices are in the tree.

### Kruskal's Algorithm
    * finds minimum spanning tree for a weighted graph.
    * greedy algorithm
    * algorithm: exact same as Prim's but cumulative distance is taken into affect (similar to Djikstra's)
